class Solution:
    def findMedianSortedArrays(self, nums1: List[int], nums2: List[int]) -> float:
        ...
        while l1 <= r1:
            ...
            print(f"l2:{l2} r2:{r2} mid2:{mid2} medium2:{cal_median(mid2, nums2)}")

            if left_index(mid2) >= 0 and right_index(mid1) < len(nums1) and nums1[right_index(mid1)] < nums2[left_index(mid2)]:
                l1 = mid1 + 1                
            elif left_index(mid1) >= 0 and right_index(mid2) < len(nums2) and nums1[left_index(mid1)] > nums2[right_index(mid2)]:
                r1 = mid1 - 1
